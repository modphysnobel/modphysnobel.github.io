...
If $C = (kx-ωt)$ and $D = [(k+Δk)x - (ω+Δω)t]$, the packet representing the superposition of waves $y_1$ and $y_2$ is expressed as $y =  A[sinC + sinD]$. Considering the trigonometric identity $sinC + sinD = 2sin[½(C+D)]cos[½(C-D)]$, we have the following relationships 
$C + D = [(2k+Δk)x - (2ω+Δω)t ]$ and $C - D = [(-Δk)x - (-Δω)t] = - [(Δk)x - (Δω)t]$. Since $cos(-θ) = cos(θ)$, then $y = 2Asin[(k+½Δk)x - (ω+½Δω)t]cos[(½Δk)x - (½Δω)t]$.
...
By accepting that in quantum physics there is no sharp separation between waves and corpuscles, we are faced with two disturbing circumstances: (1) when light interacts with matter it loses its wave characteristics in connection to its propagation and quantized impact and (2) when different components of matter interact with radiation, they lose their corpuscular properties and show diffraction. Therefore, what kind of motion equation will satisfy the matter waves proposed by de Broglie?
...
The answer to the last question was given in 1926 by Erwin Rudolf Josef Alexander Schrödinger (1887 - 1961). To get an idea of what Schrödinger's equation means, it is convenient to refer to Fermat's and Hamilton's principles.
...
Remember that matter matter waves associated to electrons have wave wavelengths of the order of the Bohr´s radio $λ≈ a_0  = 10^{-10}  m=1 Å$. Then, why a truck does not diffract as a quantum particle? Because their corresponding wavelength  $λ_{truck}≪a_0$. Consider a truck of mass 3,600 kg with a velocity of 100 km/hr, then its linear momentum $p=mv=$(3,600 kg)(100 km/hr)$= 10^5$ kg m/s; the corresponding wavelength  $λ=  \frac{h}{p}=  \frac{6.63 \times 10^{-34}  J.s}{10^5 kg m/s}= 6.63^{-39} m$, which is practically null and undetectable by conventional physical media. 

*Principles of correspondence and uncertainty*.

In the quantum world, a wave-particle duality is manifested; furthermore, causality is not strict, nor determinism is absolute. The representations and interpretations of microscopic entities contrast strongly with what is proposed by classical physics. In the quantum world, new principles apply, such as the Bohr´s correspondence principle and Heisenberg's uncertainty principle. 

Correspondence principle indicates that classical results are limiting cases of quantum results when the value of Planck´s constant is strictly negligeable.  Indeterminacy or uncertainty principle refers to an inequality satisfied by the product of the uncertainties of two physical quantities that is equal or lesser than $h$. For instance: the lack of precision in the measurement of the position of a moving particle multiplied by the lack of precision in the measurement of the component of the linear momentum in the same direction of the displacement must be equal or lesser than $h$. Both measurements must simultaneously be made on the same particle, in the same place and at the same time.
...
Up to now everything has been a classic treatment. Using the value obtained by Bohr $r = a_0(n^2)$, where $a_0 = [(ђ^2)/(mk)]$, then the frequency of an electronic orbit will be $ν_{class} = (1/2π)[k/m]^{½}[(a_0^3)(n^6)]^{-½} = (1/2π)[k/m]^{1/2} [(mk / ђ^2)]^{3/2} (n^{-3}) = (1/2π)[(mk^2)/(ђ)^3](n^{-3})$.	
...
[**9.3.  Communication formats for describing scientific texts.**](https://github.com/modphysnobel/pbook1/blob/main/docs/vol-III/vol-III-chap-9-sect-3.md)
